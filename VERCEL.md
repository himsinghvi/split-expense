...
`app/main.py` runs `Base.metadata.create_all` on startup. `app/db_migrate.py` applies **SQLite-only** legacy migrations, then **`ensure_organization_contribution_expense_id`** and **`ensure_expense_pool_credit_user_id`** run on **every** database (including Postgres). That matters because `create_all` does **not** add new columns to tables that already exist—without these patches, an older Neon schema can miss `organization_contributions.expense_id` or `expenses.pool_credit_user_id` and the app can crash on cold start when code touches those columns.
